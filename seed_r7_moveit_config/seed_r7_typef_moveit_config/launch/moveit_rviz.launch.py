"""
moveit_rviz.launch.py  —  RViz2 with MoveIt2 MotionPlanning plugin for typeF
"""
...
def launch_setup(context, *args, **kwargs):
    ...
    moveit_share = get_package_share_directory('seed_r7_typef_moveit_config')
    ...
    moveit_config = (
        MoveItConfigsBuilder(
            'SEED-Noid-Mover-typeF',
            package_name='seed_r7_typef_moveit_config',
        )
        .robot_description(file_path=xacro_file, mappings=xacro_mappings)
        .robot_description_semantic(file_path='config/SEED-Noid-Mover-typeF.srdf')
    ...
    if rviz_config:
        ...
        rviz_config_file = os.path.join(moveit_share, 'launch', 'moveit.rviz')
...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument('robot_model',     default_value='typef'),
